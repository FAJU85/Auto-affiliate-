...
describe('optimiseImage', () => {
  it('returns null when given null', async () => {
    const result = await optimiseImage(null);
    assert.equal(result, null);
  });

  it('returns a Buffer when given a valid JPEG buffer', async () => {
    ...
    const minimalJpeg = Buffer.from([
      ...
      0x00,0x08,0x01,0x01,0x00,0x00,0x3F,0x00,0xFB,0xDE,0xBF,0xFF,0xD9,
    ]);
    const result = await optimiseImage(minimalJpeg);
    assert.ok(Buffer.isBuffer(result), 'returns a Buffer');
    assert.ok(result.length > 0, 'buffer is non-empty');
  });

  it('returns original buffer on invalid image data (fail-open)', async () => {
