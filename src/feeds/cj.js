...
function pickLink(links) {
  ...
  // Product Links deep-link to a specific product page — strongly prefer these
  // so the link destination matches the product name in the post.
  // Text Links and Banners are usually brand/category level.
  const productLinks = valid.filter(l => String(l['link-type'] || '').toLowerCase() === 'product link');
  const withCommission = (productLinks.length > 0 ? productLinks : valid).filter(l => {
    const commStr = l['click-commission'] || l['sale-commission'] || '0';
    return parseFloat(commStr.replace('%', '')) > 0;
  });
  const pool = withCommission.length > 0 ? withCommission
